...
bool Scene101::init()
{
	...
	auto label2 = Label::createWithBMFont("fonts/0.fnt", str1);
	auto label3 = Label::createWithBMFont("fonts/0.fnt", str2);
	...
	// add Bean Button
	this->_bean = Sprite::create("Scene101/bean1_01.png");
	size = _bean->getContentSize();
	this->_bean->setPosition(Vec2(origin.x + size.width / 2 + 230, origin.y + visibleSize.height - size.height / 2 - 25));
	pos = _bean->getPosition();
	this->_rectbean = Rect(pos.x - size.width / 2, pos.y - size.height / 2, size.width, size.height);
	this->addChild(_bean, 1);
...
void  Scene101::onTouchEnded(cocos2d::Touch *pTouch, cocos2d::Event *pEvent) //觸碰結束事件 
{
	...
	if (_touch) {
		...
		_bean->setPosition(touchLoc);
		pos = _bean->getPosition();
		_rectbean = Rect(pos.x - size.width / 2, pos.y - size.height / 2, size.width, size.height);
		_touch = false;
	}
